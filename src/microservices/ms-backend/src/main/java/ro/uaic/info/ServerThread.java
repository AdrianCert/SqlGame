...
public class ServerThread implements Runnable {
     ...
     */
    private final static Collection<Class<? extends DispatcherController>> dispatcherControllers = Stream
            .of(UserTableController.class, BoardController.class, BoardMembershipController.class,
                    BoardPublishController.class, HistoryController.class, PaymentController.class,
    ...
    public void run() {
        ...
        try {
            ...
            httpResponse.addHeader(HttpMessage.HEADER.ACCESS_CONTROL_ALLOW_ORIGIN, "*");
            httpResponse.addHeader(HttpMessage.HEADER.ACCESS_CONTROL_ALLOW_METHODS, "*");
            httpResponse.addHeader(HttpMessage.HEADER.ACCESS_CONTROL_MAX_AGE, "3000");
            httpResponse.addHeader(HttpMessage.HEADER.ACCESS_CONTROL_ALLOW_HEADERS, "*");
    ...
    /**
     * Iterate through declared DispatcherController locking for one controller
     * responsible. The responsible controller is found by it mapping filed ( regex
     * component): mapRegex
     * 
    ...
    private HttpMessageResponse NotFound() {
        Character quo = '"';
        return new HttpMessageResponseBuilder().status(404).header(HttpMessage.HEADER.CONTENT_TYPE, "text/json")
                ...
                .body(new StringBuilder().append('{')
                        // add timestamp
                        .append(quo).append("timestamp").append(quo).append(':').append(quo)
                        .append(new Date().toString()).append(quo).append(',')
                        // add path
                        .append(quo).append("path").append(quo).append(':').append(quo).append(httpRequest.getUri())
                        .append(quo).append(',')
                        // add status
                        .append(quo).append("status").append(quo).append(':').append(quo).append("404").append(quo)
                        .append('}').toString());
    }
    ...
    private HttpMessageResponse InternalServerError(String message) {
        Character quo = '"';
        return new HttpMessageResponseBuilder().status(505).header(HttpMessage.HEADER.CONTENT_TYPE, "text/json")
                .header(HttpMessage.HEADER.CONNECTION, httpRequest.getHeader(HttpMessage.HEADER.CONNECTION))
                .header(HttpMessage.HEADER.HOST, httpRequest.getHeader(HttpMessage.HEADER.HOST))
                .body(new StringBuilder().append('{')
                        // add timestamp
                        ...
                        .append(new Date().toString()).append(quo).append(',')
                        // add message
                        .append(quo).append("message").append(quo).append(':').append(quo).append(message).append(quo)
                        .append(',')
                        // add path
                        .append(quo).append("path").append(quo).append(':').append(quo).append(httpRequest.getUri())
                        .append(quo)
                        // add status
                        .append(quo).append("status").append(quo).append(':').append(quo).append("505").append(quo)
                        .append('}').toString());
    }
}
